...
export default function checkCompleteBlock(statement: string): ExecuteOutput {
    ...
    if (flag<4){
        ...
        if(statement === "START" && flag === 4){ 
            flag = 5; //sets the flag to 3 to indicate that start is already implemented
        }
        else if(statement === "STOP" && flag===4){
            flag = 1; //sets the flag to 1 that start is first seen in the code
        }
        else if(statement === "STOP" && flag===5){
            flag = 1; //sets the flag to 1 that start is first seen in the code
        }
    }
    console.log(flag)
